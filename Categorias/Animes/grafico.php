...
<head>
    ...
    <script src="grafico.js"></script> <!-- Script externo -->
        ...
        <nav class="nav_container">
            ...
            <div class="nav_elementos">
                <ul class="nav_ul">
                    ...
                    <li class="nav_li categorias"><a href="/Categorias/categorias.html" class="link">Categorias</a><span
                            class="material-symbols-outlined dropdown_seta">arrow_drop_up</span>
...
<?php
// Lê votos salvos no arquivo 'votos.txt'
$arquivo = 'votos.txt';
$dados = file($arquivo, FILE_IGNORE_NEW_LINES);

// Conta quantos votos cada anime recebeu
$contagem = [];
foreach ($dados as $linha) {
    list(, $anime) = explode(";", $linha);
    // Divide a linha para pegar só o nome do anime (segundo campo)
    if (!isset($contagem[$anime])) {
        // Se ainda não existe esse anime no array, inicia com zero
        $contagem[$anime] = 0;
    }
    $contagem[$anime]++;
    // Incrementa o número de votos para esse anime
}

// Prepara os dados para o gráfico em formato JSON (listas de labels e valores)
$labels = json_encode(array_keys($contagem));
$valores = json_encode(array_values($contagem));

// Chama a função JavaScript 'montarGrafico' que desenha o gráfico passando os dados
